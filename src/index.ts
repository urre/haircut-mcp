...
server.tool(
  "get-haircut-times",
  "Get available appointment times from BokaDirekt",
  async ({}) => {
  try {
    ...
    const availableTimes: TimeSlot[] = data?.fromErp || [];

    // Process available times to make them more user-friendly
    const formattedTimes: FormattedTimeSlot[] = availableTimes.map((slot: TimeSlot) => {
      // Parse the date
    ...
    const groupedByDate: GroupedSlots = formattedTimes.reduce((acc: GroupedSlots, slot: FormattedTimeSlot) => {
      if (!acc[slot.date]) {
        acc[slot.date] = [];
      }
    ...
    return {
      ...
      structuredContent: {
        ...
        priceRange: availableTimes.length > 0 ? {
          min: Math.min(...availableTimes.map((slot: TimeSlot) => slot.employeePrices.price)),
          max: Math.max(...availableTimes.map((slot: TimeSlot) => slot.employeePrices.price)),
          currency: "SEK"
        } : null,
